...
```bash
covid19-data-retriever-function$ sam build --use-container
```
...
```bash
covid19-data-retriever-function$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
covid19-data-retriever-function$ sam local start-api
covid19-data-retriever-function$ curl http://localhost:3000/
```
...
```bash
covid19-data-retriever-function$ sam logs -n HelloWorldFunction --stack-name covid19-data-retriever-function --tail
```
...
Tests are defined in the `tests` folder in this project. Use PIP to install the [pytest](https://docs.pytest.org/en/latest/), [LocalStack](https://github.com/localstack/localstack) and run unit tests.
...
```bash
aws cloudformation delete-stack --stack-name covid19-data-retriever-function
```
